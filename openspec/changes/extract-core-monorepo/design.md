...
The editor is currently a single npm package (`@juanmendez90/docx-js-editor`) with ~80% framework-agnostic code and ~20% React UI. The codebase already has clean internal boundaries — separate entry points (`core.ts`, `headless.ts`, `react.ts`, `ui.ts`) and framework-agnostic directories (`src/docx/`, `src/types/`, `src/prosemirror/`, `src/layout-engine/`, `src/layout-painter/`, `src/layout-bridge/`). A community contributor wants to build a Vue wrapper, which requires the core to be importable without React dependencies.
...
- Extract framework-agnostic core into `@juanmendez90/docx-core` with zero React dependencies
- Keep `@juanmendez90/docx-js-editor` as the React UI package (preserves npm stats, no migration for existing users)
- Use Bun workspaces for monorepo management (already using Bun as runtime)
- Enable community Vue/Svelte wrappers that depend only on `@juanmendez90/docx-core`
...
**Decision:** Split into exactly `@juanmendez90/docx-core` and `@juanmendez90/docx-js-editor` (React).
...
**Decision:** `@juanmendez90/docx-js-editor` stays as the React package name. No rename.
...
**React (`@juanmendez90/docx-js-editor`):**
...
**Decision:** React package imports from `@juanmendez90/docx-core` (the npm package name), not relative paths.
...
**Core interface (`@juanmendez90/docx-core`):**
...
**React adapter (`@juanmendez90/docx-js-editor`):**
...
**Vue adapter (`@juanmendez90/docx-editor-vue`):**
...
- `EditorPluginCore`, `PanelConfig`, `PluginPanelProps` (without ReactNode), `RenderedDomContext` → `@juanmendez90/docx-core`
...
**[Risk] Internal import churn** → Files that currently do `../docx/parser` will need `@juanmendez90/docx-core` imports. Mitigation: batch find-and-replace, verify with typecheck.
