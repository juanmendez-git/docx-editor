...
The `packages/core/` directory SHALL contain all framework-agnostic source code with its own `package.json` named `@juanmendez90/docx-core`.
...
The `packages/react/` directory SHALL contain all React-dependent source code with its own `package.json` named `@juanmendez90/docx-js-editor`.
...
- **THEN** `@juanmendez90/docx-core` SHALL be listed in dependencies
...
- **AND** it SHALL externalize `@juanmendez90/docx-core` (not bundle it)
...
- **THEN** tests SHALL import from both `@juanmendez90/docx-core` and `@juanmendez90/docx-js-editor`
...
- **AND** the app SHALL render a functional DOCX editor using `@juanmendez90/docx-editor-vue` and `@juanmendez90/docx-core`
...
The `packages/vue/` directory SHALL contain an empty scaffolded package named `@juanmendez90/docx-editor-vue` with a dependency on `@juanmendez90/docx-core`, ready for community contribution.

#### Scenario: Vue package has minimal package.json

- **WHEN** inspecting `packages/vue/package.json`
- **THEN** it SHALL have name `@juanmendez90/docx-editor-vue`
- **AND** `@juanmendez90/docx-core` SHALL be listed in dependencies
