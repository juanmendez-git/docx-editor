...
- **BREAKING**: Split the single `@juanmendez90/docx-js-editor` package into a Bun workspaces monorepo with separate packages
- New `@juanmendez90/docx-core` package containing all framework-agnostic code: `src/docx/`, `src/types/`, `src/prosemirror/`, `src/layout-engine/`, `src/layout-painter/`, `src/layout-bridge/`, `src/core-plugins/`, `src/utils/` (non-React parts), `src/agent/`, `src/mcp/`
- `@juanmendez90/docx-js-editor` becomes a thin React UI package depending on `@juanmendez90/docx-core`, containing: `src/components/`, `src/hooks/`, `src/plugin-api/`, `src/plugins/`, `src/paged-editor/`
- Remove React type leaks from core code paths (e.g., `CSSProperties` imports in utils)
- Split `EditorPlugin` interface into a framework-agnostic core (`EditorPluginCore` in `@juanmendez90/docx-core`) and framework-specific adapters (`ReactEditorPlugin` in React package, `VueEditorPlugin` in Vue package)
- Existing `@juanmendez90/docx-js-editor` keeps its npm name and download stats — existing users continue using it as before
...
- `core-package-api`: Public API surface for `@juanmendez90/docx-core` — exports for DOCX parsing/serialization, ProseMirror schema/extensions/commands, layout engine, document model types, and headless editor creation
...
- **npm**: New `@juanmendez90/docx-core` package published; `@juanmendez90/docx-js-editor` gains `@juanmendez90/docx-core` as dependency; `@juanmendez90/docx-editor-vue` scaffolded for community contribution
...
- **Downstream**: Vue/Svelte/vanilla consumers can depend on `@juanmendez90/docx-core` directly with zero React overhead
