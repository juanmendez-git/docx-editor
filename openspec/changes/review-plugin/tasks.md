...
- [x] 1.1 Create `packages/agent-use/` with `package.json` (`@juanmendez90/docx-editor-agents`, BSL-1.1, peer dep on `@juanmendez90/docx-core`), `tsconfig.json`, `tsup.config.ts` with two entry points (`src/index.ts`, `src/bridge.ts`)
