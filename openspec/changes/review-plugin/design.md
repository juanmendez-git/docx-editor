...
Monorepo: `packages/core` (`@juanmendez90/docx-core`, MIT), `packages/react`, `packages/vue`. Root workspace `"packages/*"`. Build: tsup. New package: `packages/agent-use` → `@juanmendez90/docx-editor-agents` (BSL-1.1).
...
`packages/agent-use/` with `@juanmendez90/docx-core` as peer dep. Published on public npm under BSL-1.1.
